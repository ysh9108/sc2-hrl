...
class BotAI(object):
    ...
    def __init__(self, is_render=False):
        ...
        if is_render:
            ...
            self.viewer_thread = FeatureViewer(self.obs_queue)
            self.viewer_thread.setDaemon(True)
    ...
    async def render(self):
        ...
        if self.obs_queue.empty():
            self.obs_queue.put(self.state)

        pass
        # if self._screen is None:
        #     self._screen = pygame.display.set_mode((1280, 700), DOUBLEBUF)
        #
        ...
        # image = self.state.feature_layer['screen']['unit_type'].rgb
