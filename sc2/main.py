...
async def _play_game_human(client, player_id, realtime, game_time_limit, feature_setting):
    while True:
...
async def _play_game_ai(client, player_id, ai, realtime, step_time_limit, game_time_limit, feature_setting):
    ...
    while True: # Episode Loop
        ...
        while True: # Game Loop
            state = await client.observation()
            gs = GameState(state.observation, game_data, game_info, feature_setting)
...
#               return Result.Tie
            ...
            try:
                ...
                # FeatureLayer 렌더링을 위해 추가됨
                if ai.is_render:
                    asyncio.ensure_future(ai.render())
...
async def _play_game(player, client, realtime, portconfig, step_time_limit=None, game_time_limit=None, feature_setting=None):
    assert isinstance(realtime, bool), repr(realtime)

    player_id = await client.join_game(player.race, portconfig=portconfig, feature_setting=feature_setting)
    logging.info(f"Player id: {player_id}")

    if isinstance(player, Human):
        result = await _play_game_human(client, player_id, realtime, game_time_limit, feature_setting)
    else:
        result = await _play_game_ai(client, player_id, player.ai, realtime, step_time_limit, game_time_limit, feature_setting)
...
async def _host_game(map_settings, players, realtime, portconfig=None, save_replay_as=None, step_time_limit=None, game_time_limit=None, feature_setting=None):

    assert len(players) > 0, "Can't create a game without players"
    assert feature_setting, "Can't run client without 'feature_setting'"
    ...
    async with SC2Process() as server:
        ...
        try:
            result = await _play_game(players[0], client, realtime, portconfig, step_time_limit, game_time_limit, feature_setting)
