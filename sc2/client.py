...
class Client(Protocol):
    ...
    async def join_game(self, race=None, observed_player_id=None, portconfig=None, feature_setting=None):

        opts_feature_layer = sc_pb.SpatialCameraSetup(
            width=feature_setting.width,
            resolution=common_pb.Size2DI(x=feature_setting.screen[0], y=feature_setting.screen[0]),
            minimap_resolution=common_pb.Size2DI(x=feature_setting.minimap[0], y=feature_setting.minimap[0]),
        )

        ifopts = sc_pb.InterfaceOptions(raw=True, score=True, feature_layer=opts_feature_layer)
